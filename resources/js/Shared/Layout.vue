  ...
  <div>
    ...
    <div class="md:flex md:flex-col">
      <div class="md:flex md:flex-col md:h-screen">
        <div class="bg-indigo-900 md:flex md:shrink-0">
          ...
          <div class="flex items-center justify-between w-full p-4 text-sm text-white border-b md:text-md md:px-12 md:py-0">
            <div class="mt-1 mr-4">{{ auth.user.account.name }}</div>
            <dropdown class="mt-1 " placement="bottom-end">
              <template #default>
                <div class="flex items-center text-white cursor-pointer select-none group">
                  <div class="mr-1 group-hover:text-indigo-600 focus:text-indigo-600 whitespace-nowrap">
                    <span>{{ auth.user.first_name }}</span>
                    <span class="hidden md:inline">&nbsp;{{ auth.user.last_name }}</span>
                  </div>
                  <icon class="w-5 h-5 fill-white group-hover:fill-indigo-600 focus:fill-indigo-600" name="cheveron-down" />
                </div>
              </template>
              <template #dropdown>
